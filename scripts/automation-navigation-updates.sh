...
check_kubectl_installed() {
	...
	command -v oc >/dev/null 2>&1 || {
		echo >&2 "oc is not installed... Aborting."
		exit 1
	}
...
import_to_navigation_file() {
	...
	feedback=$(bash -c 'oc get navconfigurations.foundation.ibm.com multicluster-hub-nav -n kube-system -o yaml > navconfigurations.original' 2>&1)
	echo $feedback
...
apply_new_items() {
	...
	feedback=$(bash -c 'oc apply -n kube-system -f navconfigurations.yaml --validate=false' 2>&1)
