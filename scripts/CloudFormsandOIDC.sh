...
ENTITLED_REGISTRY_SECRET=`oc -n ibm-cp4mcm get secret ibm-entitlement-key -o jsonpath='{.data.\.dockerconfigjson}' | base64 -d`

echo $ENTITLED_REGISTRY_SECRET
...
# Wait for install
#
echo "Waiting for installation to start. (180 seconds)"
sleep 180
...
rm navconfigurations.original
rm navconfigurations.yaml
